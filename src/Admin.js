...
function ProductsSection() {
  ...
  const save=async()=>{
    ...
    if(editing){
      ...
      if(!error){notify("✓ Produit ajouté ! Vous pouvez maintenant uploader une image.");setSavedId(data.id);setEditing(data.id);}
  ...
  const toggle=async(p)=>{
    ...
    notify(p.active?"⏸ Produit désactivé":"▶ Produit réactivé",p.active?C.orange:C.green);
  };
  const remove=async(id)=>{await sb.from("products").delete().eq("id",id);notify("🗑️ Produit supprimé",C.red);};
      ...
      {showForm&&(
        <div style={{background:C.card,border:`1px solid ${C.gold}44`,borderRadius:18,padding:24,marginBottom:22,animation:"fadeUp .3s ease"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:14,marginBottom:14}}>
            ...
            <div>
              <label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:5}}>Type réservation</label>
          ...
          <div style={{display:"flex",gap:10,marginBottom:20}}>
            <button onClick={save} disabled={saving||!form.name||!form.price} style={{background:saving||!form.name||!form.price?"#333":`linear-gradient(135deg,${C.goldD},${C.gold})`,color:saving||!form.name||!form.price?C.muted:C.bg,border:"none",borderRadius:12,padding:"12px 24px",fontWeight:700,fontSize:14,cursor:saving?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:8}}>
              {saving?<><Spinner size={16}/>Sauvegarde…</>:"💾 Sauvegarder"}
            </button>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(310px,1fr))",gap:14}}>
          {filtered.map(p=>(
            <div key={p.id} style={{background:C.card,border:`1px solid ${p.active?C.border:"#111"}`,borderRadius:16,padding:18,display:"flex",gap:14,alignItems:"center",opacity:p.active?1:0.45}}>
              <div style={{width:64,height:64,borderRadius:14,background:C.card2,border:`1px solid ${C.border}`,overflow:"hidden",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                {p.image_url ? <img src={p.image_url} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover"}}/> : <span style={{fontSize:30}}>{p.emoji}</span>}
              </div>
              <div style={{flex:1,minWidth:0}}>
                ...
                <p style={{fontSize:11,color:C.muted,marginBottom:5}}>{CATS.find(c=>c.id===p.cat)?.label}{!p.active&&" · 🔴 Masqué"}{!p.image_url&&" · 📷 Pas d'image"}</p>
...
function OrdersSection() {
  ...
  return (
    <div>
      ...
      {loading?<div style={{display:"flex",gap:10,alignItems:"center"}}><Spinner/><span style={{color:C.muted}}>Chargement…</span></div>:filtered.length===0?<p style={{color:C.muted,fontSize:14,padding:"40px 0",textAlign:"center"}}>Aucune commande pour l'instant.</p>:(
...
function ReservationsSection() {
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:18,flexWrap:"wrap"}}>
        {types.map(t=>(
          <button key={t} onClick={()=>setFilter(t)} style={{padding:"8px 14px",borderRadius:999,border:`1.5px solid ${filter===t?C.gold:C.border}`,background:filter===t?`${C.gold}18`:"transparent",color:filter===t?C.gold:C.muted,fontWeight:600,fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>{t==="Tous"?"Tous":CATS.find(c=>c.id===t)?.label||t}</button>
        ))}
      </div>
...
function StatsSection() {
  ...
  const resRev=reservations.filter(r=>r.status==="Confirmée").reduce((s,r)=>s+r.total,0);
  const byMethod=["FedaPay","Stripe","PayPal","Systeme.io"].map(m=>({label:m,count:orders.filter(o=>o.pay_method===m).length,rev:orders.filter(o=>o.pay_method===m&&o.status!=="Annulé").reduce((s,o)=>s+o.total,0)}));
  return (
    <div>
      <div style={{marginBottom:22}}>
          ...
          {byMethod.map((m,i)=>(
            <div key={m.label}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                ...
                <span style={{fontSize:13,fontWeight:800,color:[C.orange,C.stripe,C.paypal,C.sys][i]}}>{m.count} commandes · {fmt(m.rev)}</span>
              </div>
              <div style={{height:10,background:C.card2,borderRadius:999,overflow:"hidden"}}>
                <div style={{height:"100%",width:orders.length?`${(m.count/orders.length)*100}%`:"0%",background:`linear-gradient(90deg,${[C.orange,C.stripe,C.paypal,C.sys][i]}88,${[C.orange,C.stripe,C.paypal,C.sys][i]})`,borderRadius:999,transition:"width 1s ease"}}/>
              </div>
            </div>
          ))}
        </div>
...
function MessagesSection() {
  ...
  const markRead=async id=>{await sb.from("messages").update({read:true}).eq("id",id);setMessages(l=>l.map(m=>m.id===id?{...m,read:true}:m));};
    ...
    <div style={{display:"grid",gridTemplateColumns:"320px 1fr",gap:20,height:"72vh"}}>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:18,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        <div style={{padding:"18px 20px",borderBottom:`1px solid ${C.border}`}}>
          ...
          <p style={{color:C.muted,fontSize:13}}>{messages.filter(m=>!m.read).length} non lu(s)</p>
        </div>
        ...
        {selected?(
          <>
            ...
            <div style={{flex:1,padding:24,overflowY:"auto"}}>
              ...
              {selected.reply_text&&<div style={{background:`${C.green}10`,border:`1px solid ${C.green}33`,borderRadius:14,padding:"14px 18px"}}><p style={{fontSize:12,color:C.green,fontWeight:700,marginBottom:6}}>✓ Votre réponse :</p><p style={{fontSize:14,color:C.white}}>{selected.reply_text}</p></div>}
            </div>
            <div style={{padding:"16px 24px",borderTop:`1px solid ${C.border}`}}>
              <textarea value={reply} onChange={e=>setReply(e.target.value)} placeholder={`Répondre à ${selected.from_name}…`} rows={3} style={{width:"100%",background:C.card2,border:`1.5px solid ${C.border}`,borderRadius:12,padding:"12px 16px",color:C.white,fontSize:14,fontFamily:"'DM Sans',sans-serif",outline:"none",resize:"none",boxSizing:"border-box",marginBottom:10}}/>
              <button onClick={sendReply} disabled={!reply.trim()||sending} style={{background:!reply.trim()||sending?"#333":`linear-gradient(135deg,${C.goldD},${C.gold})`,color:!reply.trim()||sending?C.muted:C.bg,border:"none",borderRadius:12,padding:"11px 24px",fontWeight:700,fontSize:14,cursor:!reply.trim()||sending?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif",display:"flex",alignItems:"center",gap:8}}>
                {sending?<><Spinner size={16}/>Envoi…</>:"✦ Envoyer"}
              </button>
            </div>
          </>
        ):(
...
function CategoriesSection() {
  ...
  const startNew = () => { setEditing(null); setForm({id:"",label:"",icon:""}); setShowForm(true); };

  const inp3 = (field, label, placeholder) => (
    <div>
      <label style={{fontSize:12,fontWeight:700,color:C.muted,display:"block",marginBottom:5}}>{label}</label>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
        <div>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontWeight:900,fontSize:24,marginBottom:4}}>Gestion des catégories</h2>
          <p style={{color:C.muted,fontSize:14}}>{cats.length} catégories · Ajoute, modifie ou supprime</p>
        </div>
        <button onClick={startNew} style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:"none",borderRadius:12,padding:"11px 22px",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>+ Nouvelle catégorie</button>
      </div>

      {showForm&&(
        <div style={{background:C.card,border:`1px solid ${C.gold}44`,borderRadius:18,padding:24,marginBottom:22,animation:"fadeUp .3s ease"}}>
          <h3 style={{fontFamily:"'Playfair Display',serif",fontWeight:800,fontSize:18,color:C.gold,marginBottom:18}}>{editing?"Modifier la catégorie":"Nouvelle catégorie"}</h3>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,marginBottom:18}}>
            {inp3("id","ID unique (sans espace)","ex: bijoux")}
            {inp3("label","Nom affiché","ex: Bijoux & Accessoires")}
            {inp3("icon","Emoji icône","ex: 💍")}
          </div>
          <div style={{background:`${C.blue}15`,border:`1px solid ${C.blue}33`,borderRadius:12,padding:"12px 16px",marginBottom:18}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
        {cats.map((c,i)=>(
          <div key={c.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:16,padding:"20px 18px",display:"flex",alignItems:"center",gap:16,animation:`fadeUp .3s ease ${i*.05}s both`}}>
            <div style={{width:56,height:56,borderRadius:14,background:`${C.gold}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>{c.icon}</div>
            <div style={{flex:1}}>
              <p style={{fontWeight:700,fontSize:15,color:C.white,marginBottom:3}}>{c.label}</p>
              <p style={{fontSize:12,color:C.muted}}>ID : <code style={{background:C.card2,padding:"1px 6px",borderRadius:5,fontSize:11}}>{c.id}</code></p>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              <button onClick={()=>startEdit(c)} style={{background:C.card2,border:`1px solid ${C.border}`,color:C.gold,borderRadius:8,padding:"5px 11px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif"}}>✏️ Éditer</button>
              <button onClick={()=>remove(c.id)} style={{background:"none",border:`1px solid ${C.red}44`,color:C.red,borderRadius:8,padding:"5px 11px",cursor:"pointer",fontSize:11,fontWeight:700,fontFamily:"'DM Sans',sans-serif"}}>🗑️ Suppr.</button>
            </div>
          </div>
        ))}
      </div>
...
export default function SMallAdmin() {
  ...
  if(!auth) return (
    <div style={{background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=DM+Sans:wght@400;500;600;700&display=swap');*{box-sizing:border-box;margin:0;padding:0;}@keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}@keyframes glow{0%,100%{box-shadow:0 0 14px ${C.gold}55}50%{box-shadow:0 0 30px ${C.gold}99}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}`}</style>
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:28,padding:"52px 48px",width:"100%",maxWidth:400,textAlign:"center",animation:"fadeUp .5s ease"}}>
        ...
        <p style={{color:C.muted,fontSize:14,marginBottom:32}}>Panneau de gestion privé</p>
