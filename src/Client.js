...
const C = {
  bg:'#080808', card:'#0f0f0f', card2:'#161616', card3:'#1e1e1e',
  gold:'#c8a84b', goldL:'#e2c46a', goldD:'#8a6820', goldXL:'#f5d98a',
  white:'#f4ede0', muted:'#8a8278', muted2:'#5a5450',
  red:'#e05a4e', green:'#4caf7d', orange:'#f59e0b', blue:'#3b82f6',
  border:'#242424', borderOr:'rgba(200,168,75,0.18)',
};
...
const ZONE_COUNTRY = {
  'Cotonou':'bj','Bénin (hors Cotonou)':'bj','Togo':'tg',"Côte d'Ivoire":'ci','International':'bj',
};
...
::-webkit-scrollbar{width:3px;}::-webkit-scrollbar-thumb{background:#8a6820;border-radius:3px;}
...
.hov{transition:transform .25s,border-color .25s;}
.hov:hover{transform:translateY(-4px);border-color:rgba(200,168,75,0.4)!important;}
.btn{transition:filter .15s,transform .1s;}.btn:hover{filter:brightness(1.08);}.btn:active{transform:scale(0.98);}
.lnk{cursor:pointer;transition:color .2s;}.lnk:hover{color:#c8a84b!important;}
select option{background:#161616;color:#f4ede0;}
.burger-menu{position:fixed;top:0;right:0;bottom:0;width:min(320px,85vw);background:#0f0f0f;border-left:0.5px solid rgba(200,168,75,0.2);z-index:200;display:flex;flex-direction:column;padding:80px 28px 36px;gap:8px;animation:slideIn .3s ease;}
.burger-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:199;animation:overlayIn .3s ease;}
.burger-item{display:flex;align-items:center;gap:14px;padding:14px 18px;border-radius:4px;cursor:pointer;font-size:11px;font-weight:400;letter-spacing:2.5px;text-transform:uppercase;color:#8a8278;border:0.5px solid transparent;transition:all .2s;font-family:'Jost',sans-serif;}
.burger-item:hover,.burger-item.act{color:#c8a84b;background:rgba(200,168,75,0.06);border-color:rgba(200,168,75,0.15);}
.burger-dot{width:4px;height:4px;border-radius:50%;background:#c8a84b;opacity:0;transition:opacity .2s;}
.burger-item.act .burger-dot{opacity:1;}
@media(max-width:768px){.hide-mob{display:none!important;}.show-mob{display:flex!important;}.col1{grid-template-columns:1fr!important;}.hero{padding:52px 22px 44px!important;}.hero-title-size{font-size:36px!important;}}
`;

const Spin=({s=20})=>(<span style={{display:'inline-block',width:s,height:s,border:'2px solid #242424',borderTopColor:C.gold,borderRadius:'50%',animation:'spin .7s linear infinite'}}/>);
const GL=()=>(<div style={{height:0.5,background:`linear-gradient(90deg,transparent,${C.gold},transparent)`,margin:'0 0 20px'}}/>);
const CatIcon=({id,size=26})=>{const p={size,strokeWidth:1.5,color:C.gold};if(id==='mode')return<ShoppingBag {...p}/>;if(id==='tech')return<Cpu {...p}/>;if(id==='formation')return<GraduationCap {...p}/>;if(id==='circuit')return<Map {...p}/>;if(id==='voiture')return<Car {...p}/>;if(id==='appart')return<Home {...p}/>;return<Sparkles {...p}/>;};

function Notif({notif}){if(!notif)return null;return(<div style={{position:'fixed',top:16,right:16,zIndex:9999,background:C.card,border:`1px solid ${notif.color}`,color:notif.color,padding:'11px 18px',borderRadius:12,fontWeight:600,fontSize:13,boxShadow:'0 8px 28px rgba(0,0,0,.7)',animation:'fadeUp .3s ease',maxWidth:300,pointerEvents:'none'}}>{notif.msg}</div>);}

function BurgerMenu({page,cat,go,onClose}){
  const links=[{label:'Accueil',key:'home',icon:'✦'},{label:'Boutique',key:'shop',icon:'◈'},{label:'Circuits',key:'circuits',icon:'◎'},{label:'Formations',key:'formations',icon:'◇'},{label:'Contact',key:'contact',icon:'◉'}];
  const isActive=k=>{if(k==='home')return page==='home';if(k==='shop')return page==='shop'&&cat!=='formation';if(k==='formations')return page==='shop'&&cat==='formation';if(k==='circuits')return page==='circuits';if(k==='contact')return page==='contact';return false;};
  return(<><div className="burger-overlay" onClick={onClose}/><nav className="burger-menu" aria-label="Navigation mobile">
    ...
    <button type="button" onClick={onClose} style={{position:'absolute',top:18,right:20,background:'none',border:`0.5px solid ${C.border}`,color:C.muted,borderRadius:8,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18}}>×</button>
    <div style={{height:0.5,background:`linear-gradient(90deg,transparent,${C.gold},transparent)`,marginBottom:8}}/>
    {links.map(({label,key,icon})=>(<div key={key} className={`burger-item${isActive(key)?' act':''}`} onClick={()=>{go(key);onClose();}} role="button" tabIndex={0} onKeyDown={e=>e.key==='Enter'&&(go(key),onClose())} aria-current={isActive(key)?'page':undefined}>
      <div className="burger-dot"/><span style={{fontSize:10,color:C.gold,fontFamily:'monospace',opacity:0.5}}>{icon}</span><span>{label}</span>
    </div>))}
    <div style={{marginTop:'auto',paddingTop:20,borderTop:`0.5px solid ${C.border}`}}>
      <p style={{fontSize:10,color:C.muted2,letterSpacing:2,textTransform:'uppercase',lineHeight:1.8}}>S-Mall · Cotonou, Bénin<br/>© 2025 S-Group</p>
    </div>
  </nav></>);
}

function BannerSlider({items}){const[idx,setIdx]=useState(0);useEffect(()=>{setIdx(0);},[items]);useEffect(()=>{if(!items||items.length<=1)return;const t=setInterval(()=>setIdx(i=>(i+1)%items.length),5000);return()=>clearInterval(t);},[items]);if(!items||items.length===0)return null;const b=items[Math.min(idx,items.length-1)];const bUrl=b.media_url||b.url,bType=b.media_type||b.type||'image',bLink=b.link_url||b.link;if(!b||!bUrl)return null;return(<div style={{position:'relative',borderRadius:4,overflow:'hidden',cursor:bLink?'pointer':'default'}}onClick={()=>bLink&&window.open(bLink,'_blank')}>{bType==='video'?<video src={bUrl}autoPlay muted loop playsInline style={{width:'100%',maxHeight:220,objectFit:'cover',display:'block'}}/>:<img src={bUrl}alt={b.title||''}style={{width:'100%',maxHeight:220,objectFit:'cover',display:'block'}}/>}{b.title&&(<div style={{position:'absolute',bottom:0,left:0,right:0,background:'linear-gradient(transparent,rgba(0,0,0,.75))',padding:'20px 16px 12px'}}><p style={{color:'#fff',fontWeight:600,fontSize:15,fontFamily:"'Cormorant Garamond',serif"}}>{b.title}</p></div>)}{items.length>1&&(<div style={{position:'absolute',bottom:10,right:12,display:'flex',gap:5}}>{items.map((_,i)=>(<div key={i}onClick={e=>{e.stopPropagation();setIdx(i);}}style={{width:i===idx?18:6,height:6,borderRadius:999,background:i===idx?C.gold:'rgba(255,255,255,.45)',cursor:'pointer',transition:'all .2s'}}/>))}</div>)}</div>);}

function ProductCard({p,cats,i,allImages,onOpen}){const isBooking=BOOKING_CATS.includes(p.cat);const imgs=allImages[p.id]||[];const img=imgs[0]||p.image_url;const cat=cats.find(c=>c.id===p.cat);return(<div className="hov"style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,overflow:'hidden',animation:`fadeUp .3s ease ${Math.min(i,8)*.04}s both`}}><div onClick={()=>onOpen(p)}style={{height:155,position:'relative',overflow:'hidden',cursor:'pointer',background:'#0d0d0d'}}>{img?<img src={img}alt={p.name}style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>:<div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100%',fontSize:52}}>{p.emoji}</div>}<div style={{position:'absolute',bottom:0,left:0,right:0,height:60,background:'linear-gradient(transparent,rgba(8,8,8,0.8))'}}/>{p.badge&&<span style={{position:'absolute',top:10,right:10,background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,fontSize:8,fontWeight:600,padding:'3px 9px',borderRadius:2,textTransform:'uppercase',letterSpacing:1.5}}>{p.badge}</span>}{p.orig_price&&<span style={{position:'absolute',top:10,left:10,background:C.red,color:'#fff',fontSize:8,fontWeight:600,padding:'3px 9px',borderRadius:2}}>-{PCT(p.orig_price,p.price)}%</span>}{cat&&<div style={{position:'absolute',bottom:0,left:0,right:0,padding:'8px 12px'}}><span style={{fontSize:8,color:C.gold,fontWeight:500,letterSpacing:2.5,textTransform:'uppercase'}}>{cat.label}</span></div>}</div><div style={{padding:'14px 16px'}}><p style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:17,color:C.white,marginBottom:4,cursor:'pointer',lineHeight:1.2}}onClick={()=>onOpen(p)}>{p.name}</p><p style={{fontSize:11,color:C.muted,marginBottom:10,lineHeight:1.5,fontWeight:300}}>{(p.desc||p.description||'').slice(0,65)}{(p.desc||p.description||'').length>65?'…':''}</p><p style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:700,color:C.gold,marginBottom:10}}>{FCFA(p.price)}</p><button type="button"onClick={()=>onOpen(p)}className="btn"style={{width:'100%',background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'9px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase',display:'flex',alignItems:'center',justifyContent:'center',gap:6}}>{isBooking?<><Calendar size={12}/>Réserver</>:<><Eye size={12}/>Voir</>}</button></div></div>);}

function ProductModal({product,cats,allVariants,allImages,onClose,onAddToCart,onBook}){const[cur,setCur]=useState(0);const[selS,setSelS]=useState(null);const[selC,setSelC]=useState(null);const[selZ,setSelZ]=useState(null);useEffect(()=>{setCur(0);setSelS(null);setSelC(null);setSelZ(null);},[product?.id]);if(!product)return null;const variants=allVariants[product.id]||[];const imgs=allImages[product.id]||[];const allImgs=imgs.length>0?imgs:(product.image_url?[product.image_url]:[]);const isBooking=BOOKING_CATS.includes(product.cat);const cat=cats.find(c=>c.id===product.cat);const storages=[...new Set(variants.filter(v=>v.storage).map(v=>v.storage))];const sizes=[...new Set(variants.filter(v=>v.size).map(v=>v.size))];const getColors=list=>{const seen={};return list.filter(v=>v.color).reduce((acc,v)=>{if(!seen[v.color]){seen[v.color]=true;acc.push({name:v.color,hex:v.color_hex||'#888'});}return acc;},[]);};const colors=storages.length>0?(selS?getColors(variants.filter(v=>v.storage===selS)):[]):getColors(variants);const hasVariants=variants.length>0;const matched=hasVariants?variants.find(v=>(storages.length===0||v.storage===selS)&&(colors.length===0||v.color===selC)&&(sizes.length===0||v.size===selZ)):null;const price=matched?matched.price:(hasVariants?Math.min(...variants.map(v=>v.price)):product.price);const acompte=Math.round(price*0.10);const canAdd=!hasVariants||!!matched;const needs=[storages.length>0&&!selS?'une capacité':null,(storages.length===0||selS)&&colors.length>0&&!selC?'une couleur':null,sizes.length>0&&!selZ?'une taille':null].filter(Boolean);return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.95)',zIndex:1000,display:'flex',alignItems:'flex-start',justifyContent:'center',padding:16,overflowY:'auto'}}onClick={onClose}><div onClick={e=>e.stopPropagation()}style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:4,width:'100%',maxWidth:640,margin:'auto',boxShadow:'0 24px 80px rgba(0,0,0,.98)'}}><div style={{position:'relative',borderRadius:'4px 4px 0 0',overflow:'hidden',height:260,background:'#000'}}>{allImgs.length>0?<img src={allImgs[cur]}alt={product.name}style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>:<div style={{height:260,display:'flex',alignItems:'center',justifyContent:'center',fontSize:80}}>{product.emoji}</div>}{allImgs.length>1&&<><button type="button"onClick={()=>setCur(c=>c===0?allImgs.length-1:c-1)}style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',background:'rgba(0,0,0,.7)',border:`0.5px solid ${C.borderOr}`,color:C.white,borderRadius:2,width:34,height:34,cursor:'pointer',fontSize:20,display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button><button type="button"onClick={()=>setCur(c=>c===allImgs.length-1?0:c+1)}style={{position:'absolute',right:10,top:'50%',transform:'translateY(-50%)',background:'rgba(0,0,0,.7)',border:`0.5px solid ${C.borderOr}`,color:C.white,borderRadius:2,width:34,height:34,cursor:'pointer',fontSize:20,display:'flex',alignItems:'center',justifyContent:'center'}}>›</button><div style={{position:'absolute',bottom:10,left:'50%',transform:'translateX(-50%)',display:'flex',gap:5}}>{allImgs.map((_,i)=><button key={i}type="button"onClick={()=>setCur(i)}style={{width:i===cur?18:6,height:6,borderRadius:999,background:i===cur?C.gold:'rgba(255,255,255,.4)',border:'none',cursor:'pointer',padding:0,transition:'all .2s'}}/>)}</div></>}{product.orig_price&&<span style={{position:'absolute',top:10,left:10,background:C.red,color:'#fff',fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:2,zIndex:5}}>-{PCT(product.orig_price,product.price)}%</span>}{product.badge&&<span style={{position:'absolute',top:10,right:44,background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,fontSize:10,fontWeight:600,padding:'3px 9px',borderRadius:2,zIndex:5}}>{product.badge}</span>}<button type="button"onClick={onClose}style={{position:'absolute',top:10,right:10,background:'rgba(0,0,0,.7)',border:`0.5px solid ${C.borderOr}`,color:'#fff',borderRadius:2,width:32,height:32,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',zIndex:5,fontSize:18}}>×</button></div><div style={{padding:'20px 24px'}}>{cat&&<p style={{fontSize:9,color:C.gold,fontWeight:500,letterSpacing:3,textTransform:'uppercase',marginBottom:6}}>{cat.label}</p>}<h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:600,color:C.white,marginBottom:8,lineHeight:1.2}}>{product.name}</h2>{(product.desc||product.description)&&<p style={{fontSize:13,color:C.muted,lineHeight:1.8,marginBottom:12,fontWeight:300}}>{product.desc||product.description}</p>}<GL/>{storages.length>0&&(<div style={{marginBottom:14}}><p style={{fontSize:11,fontWeight:500,color:C.muted,marginBottom:8,letterSpacing:1.5,textTransform:'uppercase'}}>Capacité</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{storages.map(s=>{const minP=Math.min(...variants.filter(v=>v.storage===s).map(v=>v.price));return(<button key={s}type="button"onClick={()=>{setSelS(selS===s?null:s);setSelC(null);}}style={{padding:'7px 14px',borderRadius:2,border:`1px solid ${selS===s?C.gold:C.border}`,background:selS===s?`${C.gold}12`:C.card2,cursor:'pointer',fontFamily:"'Jost',sans-serif",display:'flex',flexDirection:'column',alignItems:'center',gap:2,transition:'all .15s'}}><span style={{fontWeight:500,fontSize:13,color:selS===s?C.gold:C.white}}>{s}</span><span style={{fontSize:10,color:C.muted}}>{FCFA(minP)}</span></button>);})}</div></div>)}{colors.length>0&&(<div style={{marginBottom:14}}><p style={{fontSize:11,fontWeight:500,color:C.muted,marginBottom:8,letterSpacing:1.5,textTransform:'uppercase'}}>Couleur{selC?<span style={{color:C.white,fontWeight:300,textTransform:'none',letterSpacing:0}}> — {selC}</span>:''}</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{colors.map(col=>(<button key={col.name}type="button"onClick={()=>setSelC(selC===col.name?null:col.name)}style={{display:'flex',alignItems:'center',gap:7,padding:'6px 12px',borderRadius:999,border:`1px solid ${selC===col.name?C.gold:'transparent'}`,background:C.card2,cursor:'pointer',transition:'all .15s',fontFamily:"'Jost',sans-serif"}}><div style={{width:16,height:16,borderRadius:'50%',background:col.hex,border:'1px solid rgba(255,255,255,.2)',flexShrink:0}}/><span style={{fontSize:12,color:selC===col.name?C.gold:C.muted,fontWeight:500}}>{col.name}</span></button>))}</div></div>)}{sizes.length>0&&(<div style={{marginBottom:14}}><p style={{fontSize:11,fontWeight:500,color:C.muted,marginBottom:8,letterSpacing:1.5,textTransform:'uppercase'}}>Taille</p><div style={{display:'flex',gap:8,flexWrap:'wrap'}}>{sizes.map(s=>(<button key={s}type="button"onClick={()=>setSelZ(selZ===s?null:s)}style={{width:44,height:44,borderRadius:2,border:`1px solid ${selZ===s?C.gold:C.border}`,background:selZ===s?`${C.gold}12`:C.card2,color:selZ===s?C.gold:C.white,fontWeight:500,fontSize:13,cursor:'pointer',fontFamily:"'Jost',sans-serif",transition:'all .15s'}}>{s}</button>))}</div></div>)}<div style={{background:C.card2,border:`0.5px solid ${matched||!hasVariants?C.gold:C.border}`,borderRadius:4,padding:'12px 16px',marginBottom:12,transition:'border-color .2s'}}><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:isBooking?8:0}}><span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:700,color:C.gold}}>{FCFA(price)}</span>{product.orig_price&&<span style={{textDecoration:'line-through',color:C.muted,fontSize:13}}>{FCFA(product.orig_price)}</span>}{matched&&<span style={{fontSize:11,color:C.green,fontWeight:500,background:`${C.green}15`,padding:'2px 9px',borderRadius:999}}>✓ Prêt</span>}</div>{isBooking&&<div style={{display:'flex',alignItems:'center',gap:7,padding:'7px 10px',background:`${C.orange}12`,border:`0.5px solid ${C.orange}33`,borderRadius:4}}><span style={{fontSize:12,color:C.orange,fontWeight:500}}>Acompte 10% requis : {FCFA(acompte)}</span></div>}</div>{hasVariants&&!matched&&needs.length>0&&<p style={{fontSize:12,color:C.orange,fontWeight:500,background:`${C.orange}10`,padding:'8px 12px',borderRadius:4,textAlign:'center',marginBottom:12}}>Veuillez sélectionner {needs.join(' et ')}</p>}{isBooking?<button type="button"className="btn"onClick={()=>canAdd&&onBook(product,price,acompte)}disabled={!canAdd}style={{width:'100%',background:canAdd?`linear-gradient(135deg,${C.goldD},${C.gold})`:'#1a1a1a',color:canAdd?C.bg:C.muted,border:'none',borderRadius:2,padding:'13px',fontWeight:500,fontSize:11,cursor:canAdd?'pointer':'not-allowed',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}><Calendar size={14}/>{canAdd?`Réserver — acompte ${FCFA(acompte)}`:'Sélectionner les options'}</button>:<button type="button"className="btn"onClick={()=>canAdd&&onAddToCart(product,matched)}disabled={!canAdd}style={{width:'100%',background:canAdd?`linear-gradient(135deg,${C.goldD},${C.gold})`:'#1a1a1a',color:canAdd?C.bg:C.muted,border:'none',borderRadius:2,padding:'13px',fontWeight:500,fontSize:11,cursor:canAdd?'pointer':'not-allowed',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase',display:'flex',alignItems:'center',justifyContent:'center',gap:8}}><ShoppingCart size={14}/>{canAdd?'Ajouter au panier':'Sélectionner les options'}</button>}</div></div></div>);}
...
export default function SMall() {
  ...
  const loadAll = useCallback(async()=>{
    try{
      ...
      if(vd.data){const vm={};vd.data.forEach(v=>{if(!vm[v.product_id])vm[v.product_id]=[];vm[v.product_id].push(v);});setAllVariants(vm);}
      if(id.data){const im={};id.data.forEach(x=>{if(!im[x.product_id])im[x.product_id]=[];im[x.product_id].push(x.url);});setAllImages(im);}
      try{const cc=await sb.from('circuit_config').select('*').single();if(cc.data)setCircuitData(cc.data);}catch{}
  ...
  const getFedaCountry = ()=>zone?ZONE_COUNTRY[zone.name]||'bj':'bj';

  const go = k => {
    setSearch('');setModal(null);setBooking(null);setBurgerOpen(false);
    if(k==='home')setPage('home');
    else if(k==='shop'){setCat('all');setPage('shop');}
    else if(k==='formations'){setCat('formation');setPage('shop');}
    else if(k==='circuits')setPage('circuits');
    else setPage(k);
  };
  const goToCat = c=>{setSearch('');setModal(null);setBooking(null);setCat(c);setPage('shop');};
  const addToCart = (p,variant)=>{
    ...
    setCart(prev=>{const ex=prev.find(i=>i._key===key);if(ex)return prev.map(i=>i._key===key?{...i,qty:i.qty+1}:i);return[...prev,{...p,price,variantLabel:vLabel,qty:1,_key:key}];});
    notify(`✦ ${p.name}${vLabel?' ('+vLabel+')':''} ajouté`);setModal(null);
  };
  ...
  const doPay = async()=>{
    ...
    try{
      if(window.FedaPay){
        const res=await fedaFetch({
          type:'order', order_id:oid, amount:snap.total,
        ...
        if(!res.ok)throw new Error('Session FedaPay echouee');
        const{token,public_key}=await res.json();
        window.FedaPay.init({public_key,transaction:{token},onComplete:async r=>{
          if(r.reason==='DIALOG DISMISSED'){setProc(false);notify('Paiement annule',C.red);return;}
          await sb.from('orders').insert({
            id:oid,client_name:form.name.trim(),client_email:form.email.trim(),
            ...
            pay_method:pay,status:'Confirme',country:snap.zone?.name||'N/A'
          });
          try{
            await fetch(EDGE,{method:'POST',headers:{'Content-Type':'application/json'},
              body:JSON.stringify({
                ...
                html:'<div style="font-family:sans-serif;background:#080808;color:#f4ede0;padding:20px;border-radius:12px;"><h1 style="color:#c8a84b;text-align:center;">S-MALL</h1><p>Bonjour '+form.name+', merci pour votre commande. Total : '+FCFA(snap.total)+'. Ref : '+oid+'</p></div>',
              })
            });
    ...
    }catch(e){setProc(false);notify('Erreur. Reessayez.',C.red);}
  };
    ...
    try{
      if(window.FedaPay){
        ...
        if(!res.ok)throw new Error('Session echouee');
        const{token,public_key}=await res.json();
        window.FedaPay.init({public_key,transaction:{token},onComplete:async r=>{
          if(r.reason==='DIALOG DISMISSED'){setProc(false);notify('Paiement annule',C.red);return;}
          await sb.from('reservations').insert({
            ...
            total:amt,acompte_recu:amt,status:'Acompte recu',
          });
          try{
            await fetch(EDGE,{method:'POST',headers:{'Content-Type':'application/json'},
              body:JSON.stringify({
                to:'agencesgroup23@gmail.com',
                subject:'Reservation '+info.product.name,
                html:'<div style="font-family:sans-serif;background:#080808;color:#f4ede0;padding:20px;"><h2 style="color:#c8a84b;">Reservation S-Mall</h2><p>'+info.product.name+' — '+info.name+' / '+info.tel+' — '+info.date+' — '+FCFA(amt)+'</p></div>',
              })
            });
          }catch(e){console.warn('Email admin failed');}
          if(info.email&&info.email!=='N/A'){
            try{
              await fetch(EDGE,{method:'POST',headers:{'Content-Type':'application/json'},
                body:JSON.stringify({
                  to:info.email,
                  subject:'Reservation confirmee — '+info.product.name,
                  html:'<div style="font-family:sans-serif;background:#080808;color:#f4ede0;padding:20px;border-radius:12px;"><h1 style="color:#c8a84b;">S-MALL</h1><p>Bonjour '+info.name+', votre acompte de '+FCFA(amt)+' pour '+info.product.name+' le '+info.date+' a ete recu. Ref : '+resId+'</p></div>',
                })
              });
            }catch(e){console.warn('Email client failed');}
          }
          setProc(false);
    ...
    }catch(e){setProc(false);notify('Erreur de paiement. Reessayez.',C.red);}
  };
  ...
  return (
    <div style={{background:C.bg,minHeight:'100vh',color:C.white,fontFamily:"'Jost',sans-serif"}}>
      <style>{CSS}</style>
      <Notif notif={notif}/>

      {/* BANDEAU COOKIES */}
      {typeof window!=='undefined'&&!localStorage.getItem('smallet_cookie_consent')&&(
        <div style={{position:'fixed',bottom:0,left:0,right:0,zIndex:10000,background:C.card,borderTop:`0.5px solid ${C.borderOr}`,padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12,boxShadow:'0 -8px 30px rgba(0,0,0,0.6)'}}>
          <div style={{flex:1,minWidth:240}}>
            <p style={{fontWeight:500,fontSize:13,color:C.white,marginBottom:3}}>Ce site utilise le stockage local</p>
            <p style={{fontSize:11,color:C.muted,lineHeight:1.7,fontWeight:300}}>Uniquement pour mémoriser votre panier. Aucun cookie publicitaire.</p>
          </div>
          <div style={{display:'flex',gap:10,flexShrink:0}}>
            <button type="button"onClick={()=>{localStorage.setItem('smallet_cookie_consent',JSON.stringify({accepted:false,date:new Date().toISOString()}));window.location.reload();}}style={{background:'none',border:`0.5px solid ${C.border}`,color:C.muted,borderRadius:2,padding:'9px 18px',fontWeight:400,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:'uppercase'}}>Refuser</button>
            <button type="button"onClick={()=>{localStorage.setItem('smallet_cookie_consent',JSON.stringify({accepted:true,date:new Date().toISOString()}));window.location.reload();}}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'9px 20px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:'uppercase'}}>Accepter</button>
          </div>
        </div>
      )}

      <ProductModal product={modal}cats={cats}allVariants={allVariants}allImages={allImages}onClose={()=>setModal(null)}onAddToCart={addToCart}onBook={openBook}/>
      <BookingModal data={booking}onClose={()=>setBooking(null)}onConfirm={doBook}/>
      {burgerOpen&&<BurgerMenu page={page}cat={cat}go={go}onClose={()=>setBurgerOpen(false)}/>}

      {/* NAV */}
      <nav style={{background:'rgba(8,8,8,0.97)',backdropFilter:'blur(16px)',padding:'0 28px',height:64,display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:`0.5px solid ${C.borderOr}`,position:'sticky',top:0,zIndex:100}}>
        ...
        <div className="hide-mob"style={{display:'flex',gap:28,alignItems:'center'}}>
          {[['Accueil','home'],['Boutique','shop'],['Circuits','circuits'],['Formations','formations'],['Contact','contact']].map(([l,k])=>(
            <span key={k}className="lnk"style={{fontFamily:"'Jost',sans-serif",fontWeight:400,fontSize:10,letterSpacing:2.5,textTransform:'uppercase',color:(page===k||(k==='formations'&&page==='shop'&&cat==='formation'))?C.gold:C.muted,paddingBottom:2,borderBottom:`0.5px solid ${(page===k||(k==='formations'&&page==='shop'&&cat==='formation'))?C.gold:'transparent'}`,transition:'all .2s'}}onClick={()=>go(k)}>{l}</span>
          ))}
        </div>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          <button type="button"className="btn"onClick={()=>setPage('cart')}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:30,padding:'8px 18px',fontWeight:500,fontSize:10,cursor:'pointer',display:'flex',alignItems:'center',gap:7,fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:'uppercase'}}>
            <ShoppingCart size={14}/>{cartN>0&&<span style={{background:'rgba(0,0,0,0.25)',borderRadius:999,padding:'1px 6px',fontSize:10,fontWeight:600}}>{cartN}</span>}Panier
          </button>
          <button type="button"className="show-mob"onClick={()=>setBurgerOpen(true)}aria-label="Ouvrir le menu"aria-expanded={burgerOpen}style={{display:'none',background:'none',border:`0.5px solid ${C.borderOr}`,borderRadius:4,width:38,height:38,cursor:'pointer',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:5,padding:10}}>
            <span style={{display:'block',width:16,height:0.5,background:C.gold}}/><span style={{display:'block',width:12,height:0.5,background:C.gold}}/><span style={{display:'block',width:16,height:0.5,background:C.gold}}/>
          </button>
        </div>
      </nav>

      {loading&&(<div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,padding:'80px 0'}}><Spin s={24}/><span style={{color:C.muted,fontSize:14,fontWeight:300}}>Chargement…</span></div>)}

      {/* HOME */}
      {!loading&&page==='home'&&(<div>
        <div className="hero"style={{position:'relative',overflow:'hidden',padding:'80px 56px 70px',background:'linear-gradient(135deg,#080808 0%,#100e08 50%,#080808 100%)'}}>
          <div style={{position:'absolute',top:-80,left:-60,width:380,height:380,borderRadius:'50%',background:`radial-gradient(circle,${C.gold}0d 0%,transparent 70%)`,pointerEvents:'none'}}/>
          <div style={{maxWidth:620,animation:'fadeUp .6s ease'}}>
            <p style={{color:C.gold,fontWeight:400,letterSpacing:5,textTransform:'uppercase',fontSize:9,marginBottom:14}}>✦ Bienvenue sur S-Mall</p>
            <h1 className="hero-title-size"style={{fontFamily:"'Cormorant Garamond',serif",fontSize:52,fontWeight:600,lineHeight:1.05,marginBottom:16}}>
              Mode. Tech. Voyages.<br/><em style={{fontStyle:'italic',fontWeight:300,background:`linear-gradient(90deg,${C.gold},${C.goldL})`,WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>Tout en un lieu.</em>
            </h1>
            <div style={{width:48,height:0.5,background:`linear-gradient(90deg,${C.gold},transparent)`,margin:'18px 0'}}/>
            <p style={{color:C.muted,fontSize:14,lineHeight:1.8,maxWidth:460,marginBottom:28,fontWeight:300}}>Vêtements, électronique, formations, circuits Bénin · Togo · CIV, voitures & appartements.</p>
            <div style={{display:'flex',gap:12,flexWrap:'wrap'}}>
              <button type="button"className="btn"onClick={()=>go('shop')}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'13px 28px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase',display:'flex',alignItems:'center',gap:8}}>Explorer<ArrowRight size={13}/></button>
              <button type="button"onClick={()=>go('circuits')}style={{background:'transparent',color:C.gold,border:`0.5px solid ${C.gold}`,borderRadius:2,padding:'13px 22px',fontWeight:400,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>Circuits</button>
            </div>
          </div>
        </div>
        ...
        <div style={{maxWidth:1180,margin:'0 auto',padding:'40px 28px'}}>
          <p style={{color:C.gold,fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,textAlign:'center',marginBottom:6}}>✦ Nos univers</p>
          <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,textAlign:'center',marginBottom:8}}>Que cherchez-vous ?</h2>
          <GL/>
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:12}}>
            {cats.filter(c=>c.id!=='all').map((c,i)=>(<div key={c.id}className="hov"onClick={()=>goToCat(c.id)}style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:'18px 12px',textAlign:'center',cursor:'pointer',animation:`fadeUp .35s ease ${i*.04}s both`}}><div style={{display:'flex',justifyContent:'center',marginBottom:8}}><CatIcon id={c.id}/></div><p style={{fontWeight:500,fontSize:11,color:C.white,letterSpacing:1}}>{c.label}</p><p style={{fontSize:10,color:C.gold,marginTop:2,fontWeight:300}}>{products.filter(p=>p.cat===c.id).length} articles</p></div>))}
          </div>
        </div>

        {promos.length>0&&(<div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 40px'}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}><Flame size={15}color={C.red}/><p style={{color:C.red,fontWeight:500,letterSpacing:3,textTransform:'uppercase',fontSize:9}}>Offres du moment</p></div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,marginBottom:8}}>Promotions</h2><GL/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:16}}>{promos.slice(0,4).map((p,i)=><ProductCard key={p.id}p={p}cats={cats}i={i}allImages={allImages}onOpen={setModal}/>)}</div></div>)}
        {midBanners.length>0&&<div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 40px'}}><BannerSlider items={midBanners}/></div>}
        {news.length>0&&(<div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 40px'}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}><Sparkles size={15}color={C.green}/><p style={{color:C.green,fontWeight:500,letterSpacing:3,textTransform:'uppercase',fontSize:9}}>Tout juste arrivé</p></div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,marginBottom:8}}>Nouveautés</h2><GL/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:16}}>{news.slice(0,4).map((p,i)=><ProductCard key={p.id}p={p}cats={cats}i={i}allImages={allImages}onOpen={setModal}/>)}</div></div>)}
        {best.length>0&&(<div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 40px'}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:5}}><TrendingUp size={15}color={C.gold}/><p style={{color:C.gold,fontWeight:500,letterSpacing:3,textTransform:'uppercase',fontSize:9}}>Les plus demandés</p></div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,marginBottom:8}}>Bestsellers</h2><GL/><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:16}}>{best.slice(0,4).map((p,i)=><ProductCard key={p.id}p={p}cats={cats}i={i}allImages={allImages}onOpen={setModal}/>)}</div></div>)}

        <div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 40px'}}>
          <p style={{color:C.gold,fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,textAlign:'center',marginBottom:6}}>✦ Témoignages</p>
          <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,textAlign:'center',marginBottom:8}}>Ce que disent nos clients</h2>
          <GL/>
          {reviews.length===0?<div style={{textAlign:'center',padding:'28px 0',color:C.muted}}><Star size={38}strokeWidth={1}style={{margin:'0 auto 10px',display:'block'}}color={C.muted}/><p style={{fontSize:13,fontWeight:400}}>Soyez le premier à laisser un avis !</p><button type="button"onClick={()=>setPage('contact')}style={{marginTop:12,background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'10px 20px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>Laisser un avis</button></div>
          :<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))',gap:14}}>{reviews.slice(0,6).map((r,i)=>(<div key={r.id}style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:'18px 20px',animation:`fadeUp .35s ease ${i*.05}s both`}}><div style={{display:'flex',justifyContent:'space-between',marginBottom:8}}><div><p style={{fontWeight:500,fontSize:14,color:C.white}}>{r.client_name}</p><p style={{fontSize:10,color:C.muted,fontWeight:300}}>{new Date(r.created_at).toLocaleDateString('fr-FR')}</p></div><div style={{display:'flex',gap:2}}>{[1,2,3,4,5].map(s=><span key={s}style={{fontSize:13,color:s<=r.rating?C.gold:'#2a2a2a'}}>★</span>)}</div></div><p style={{fontSize:13,color:C.muted,lineHeight:1.7,fontStyle:'italic',fontWeight:300}}>"{r.comment}"</p></div>))}</div>}
          <div style={{textAlign:'center',marginTop:18}}><button type="button"onClick={()=>setPage('contact')}style={{background:'transparent',color:C.gold,border:`0.5px solid ${C.gold}`,borderRadius:2,padding:'9px 22px',fontWeight:400,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>✦ Laisser un avis</button></div>
        </div>

        <div style={{maxWidth:1180,margin:'0 auto',padding:'0 28px 48px'}}>
          <div style={{background:'linear-gradient(135deg,#0f0808,#1a0e08)',border:`0.5px solid rgba(232,83,63,.2)`,borderRadius:2,padding:'28px 36px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:18}}>
            <div><p style={{color:'#e8533f',fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,marginBottom:7}}>⚡ Formations en ligne</p><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:22,fontWeight:600,marginBottom:5}}>Formations & Ebooks</h3><p style={{color:C.muted,fontSize:13,maxWidth:380,lineHeight:1.6,fontWeight:300}}>Accès envoyé automatiquement par email dès le paiement.</p></div>
            <button type="button"className="btn"onClick={()=>goToCat('formation')}style={{background:'linear-gradient(135deg,#c0392b,#e8533f)',color:'#fff',border:'none',borderRadius:2,padding:'11px 22px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",flexShrink:0,letterSpacing:2,textTransform:'uppercase'}}>Voir les formations →</button>
          </div>
        </div>
      </div>)}
      ...
      {!loading&&page==='shop'&&(<div style={{maxWidth:1180,margin:'0 auto',padding:'36px 28px',animation:'fadeUp .35s ease'}}>
        <div style={{display:'flex',gap:10,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
          <div style={{flex:1,minWidth:180,display:'flex',alignItems:'center',background:C.card,border:`0.5px solid ${C.border}`,borderRadius:4,padding:'9px 14px',gap:8}}>
            <Search size={14}color={C.gold}/>
            <input value={search}onChange={e=>setSearch(e.target.value)}placeholder="Rechercher…"style={{border:'none',outline:'none',background:'transparent',color:C.white,fontSize:13,width:'100%',fontFamily:"'Jost',sans-serif",fontWeight:300}}/>
          ...
          <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
            {cats.map(c=>(<button key={c.id}type="button"onClick={()=>setCat(c.id)}style={{padding:'7px 13px',borderRadius:999,border:`0.5px solid ${cat===c.id?C.gold:C.border}`,background:cat===c.id?`${C.gold}12`:'transparent',color:cat===c.id?C.gold:C.muted,fontWeight:500,fontSize:10,cursor:'pointer',fontFamily:"'Jost',sans-serif",transition:'all .2s',letterSpacing:1.5,textTransform:'uppercase'}}>{c.label}</button>))}
          </div>
        </div>
        <p style={{color:C.muted,fontSize:11,marginBottom:16,fontWeight:300,letterSpacing:1}}>{filtered.length} article{filtered.length!==1?'s':''}</p>
        {filtered.length===0?<div style={{textAlign:'center',padding:'60px 0',color:C.muted}}><Search size={44}strokeWidth={1}style={{margin:'0 auto 12px',display:'block'}}color={C.muted}/><p style={{fontSize:14,fontWeight:400}}>Aucun résultat</p></div>
        :<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:16}}>{filtered.map((p,i)=><ProductCard key={p.id}p={p}cats={cats}i={i}allImages={allImages}onOpen={setModal}/>)}</div>}
      </div>)}

      {/* CART */}
      {page==='cart'&&(<div style={{maxWidth:940,margin:'0 auto',padding:'36px 28px',animation:'fadeUp .35s ease'}}>
        <p style={{color:C.gold,fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,marginBottom:5}}>✦ Mon panier</p>
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:30,fontWeight:600,marginBottom:18}}>{cartN} article{cartN!==1?'s':''}</h2>
        <GL/>
        {cart.length===0?<div style={{textAlign:'center',padding:'70px 0'}}><ShoppingCart size={52}strokeWidth={1}color={C.muted}style={{margin:'0 auto 14px',display:'block'}}/><p style={{fontWeight:500,fontSize:15,color:C.muted,marginBottom:18}}>Votre panier est vide</p><button type="button"className="btn"onClick={()=>go('shop')}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'12px 26px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>Parcourir la boutique</button></div>
        :<div className="col1"style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:20,alignItems:'start'}}>
          <div style={{display:'flex',flexDirection:'column',gap:10}}>
            {cart.map((item,idx)=>(<div key={item._key||idx}style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:'14px 16px',display:'flex',alignItems:'center',gap:14}}>
              <div style={{width:50,height:50,borderRadius:4,background:C.card2,display:'flex',alignItems:'center',justifyContent:'center',fontSize:22,flexShrink:0,overflow:'hidden',border:`0.5px solid ${C.border}`}}>{item.image_url?<img src={item.image_url}style={{width:'100%',height:'100%',objectFit:'cover'}}alt=""/>:<span>{item.emoji}</span>}</div>
              <div style={{flex:1}}><p style={{fontWeight:500,fontSize:13,color:C.white,marginBottom:2}}>{item.name}</p>{item.variantLabel&&<p style={{fontSize:11,color:C.gold,marginBottom:2,fontWeight:300}}>{item.variantLabel}</p>}<p style={{color:C.gold,fontWeight:600,fontSize:13}}>{FCFA(item.price)}</p></div>
              <div style={{display:'flex',alignItems:'center',gap:7}}>
                <button type="button"onClick={()=>setCart(prev=>prev.map((it,i)=>i===idx?{...it,qty:Math.max(1,it.qty-1)}:it))}style={{width:26,height:26,borderRadius:4,border:`0.5px solid ${C.border}`,background:'transparent',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:C.muted}}><Minus size={11}/></button>
                <span style={{fontWeight:600,fontSize:13,minWidth:18,textAlign:'center'}}>{item.qty}</span>
                <button type="button"onClick={()=>setCart(prev=>prev.map((it,i)=>i===idx?{...it,qty:it.qty+1}:it))}style={{width:26,height:26,borderRadius:4,border:`0.5px solid ${C.border}`,background:'transparent',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:C.muted}}><Plus size={11}/></button>
              </div>
              <div style={{textAlign:'right',minWidth:80}}><p style={{fontWeight:700,fontSize:13,color:C.white,marginBottom:4}}>{FCFA(item.price*item.qty)}</p><button type="button"onClick={()=>setCart(prev=>prev.filter((_,i)=>i!==idx))}style={{background:'none',border:'none',color:C.red,cursor:'pointer',fontSize:11,fontFamily:"'Jost',sans-serif",display:'flex',alignItems:'center',gap:3,marginLeft:'auto'}}><Trash2 size={10}/>Retirer</button></div>
            </div>))}
            <button type="button"onClick={()=>go('shop')}style={{background:'none',border:`0.5px solid ${C.border}`,borderRadius:4,padding:'10px',fontWeight:400,fontSize:11,color:C.muted,cursor:'pointer',fontFamily:"'Jost',sans-serif",display:'flex',alignItems:'center',justifyContent:'center',gap:5,letterSpacing:1.5,textTransform:'uppercase'}}><ChevronLeft size={13}/>Continuer mes achats</button>
          </div>
          <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:20,position:'sticky',top:76}}>
            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:18,marginBottom:16}}>Récapitulatif</h3>
            <div style={{display:'flex',flexDirection:'column',gap:11}}>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:13}}><span style={{color:C.muted,fontWeight:300}}>Sous-total</span><span style={{fontWeight:600}}>{FCFA(subtotal)}</span></div>
              {needsShip&&(<div><label style={{fontSize:10,fontWeight:500,color:C.muted,display:'flex',alignItems:'center',gap:5,marginBottom:7,letterSpacing:1.5,textTransform:'uppercase'}}><MapPin size={11}/>Zone de livraison</label><select value={zone?.id||''}onChange={e=>{const z=zones.find(z=>String(z.id)===e.target.value);setZone(z||null);}}style={{width:'100%',background:C.card2,border:`0.5px solid ${zone?C.gold:C.border}`,borderRadius:4,padding:'9px 11px',color:zone?C.white:C.muted,fontSize:12,fontFamily:"'Jost',sans-serif",outline:'none',cursor:'pointer'}}><option value="">-- Choisir votre zone --</option>{zones.map(z=><option key={z.id}value={String(z.id)}>{z.name} — {z.free_above>0&&subtotal>=z.free_above?'Gratuite':FCFA(z.price)} ({z.delay})</option>)}</select></div>)}
              {needsShip&&zone&&<DeliveryMap zone={zone}/>}
              {needsShip&&<div style={{display:'flex',justifyContent:'space-between',fontSize:13}}><span style={{color:C.muted,display:'flex',alignItems:'center',gap:4,fontWeight:300}}><Truck size={11}/>Livraison</span><span style={{fontWeight:600,color:!zone?C.muted:shipCost===0?C.green:C.white}}>{!zone?'À choisir':shipCost===0?'Gratuite 🎉':FCFA(shipCost)}</span></div>}
              {zone&&zone.free_above>0&&subtotal<zone.free_above&&<p style={{fontSize:11,color:C.muted,background:'#100e08',padding:'6px 9px',borderRadius:4,fontWeight:300}}>💡 Encore {FCFA(zone.free_above-subtotal)} pour la livraison gratuite</p>}
              {zone&&<div style={{display:'flex',alignItems:'center',gap:4,fontSize:11,color:C.muted,fontWeight:300}}><Clock size={10}/>{zone.delay}</div>}
              <div style={{height:0.5,background:C.border}}/>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:18,fontFamily:"'Cormorant Garamond',serif",fontWeight:700}}><span>Total</span><span style={{color:C.gold}}>{FCFA(total)}</span></div>
            </div>
            <button type="button"className="btn"onClick={()=>setPage('checkout')}disabled={needsShip&&!zone}style={{width:'100%',marginTop:14,background:(!needsShip||zone)?`linear-gradient(135deg,${C.goldD},${C.gold})`:'#1a1a1a',color:(!needsShip||zone)?C.bg:C.muted,border:'none',borderRadius:2,padding:'12px',fontWeight:500,fontSize:11,cursor:(!needsShip||zone)?'pointer':'not-allowed',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>{needsShip&&!zone?'Choisissez une zone':'Passer la commande →'}</button>
          </div>
        </div>}
      </div>)}
      ...
      {page==='checkout'&&(<div style={{maxWidth:840,margin:'0 auto',padding:'36px 28px',animation:'fadeUp .35s ease'}}>
        <p style={{color:C.gold,fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,marginBottom:5}}>✦ Finaliser</p>
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,marginBottom:18}}>Paiement sécurisé</h2>
        <GL/>
        <div className="col1"style={{display:'grid',gridTemplateColumns:'1fr 280px',gap:20,alignItems:'start'}}>
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:20}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:16,marginBottom:14,color:C.gold}}>Vos informations</h3>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:11}}><Inp f="name"pl="Nom complet"/><Inp f="email"pl="Email"t="email"/><Inp f="tel"pl="Téléphone"t="tel"/></div>
            </div>
            {needsShip&&zone&&(<div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:18}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}><div style={{display:'flex',alignItems:'center',gap:8}}><Truck size={14}color={C.gold}/><div><p style={{fontWeight:500,fontSize:13,color:C.white}}>{zone.name}</p><p style={{fontSize:10,color:C.muted,marginTop:2,fontWeight:300}}>{zone.delay}</p></div></div><span style={{fontWeight:700,fontSize:14,color:shipCost===0?C.green:C.gold}}>{shipCost===0?'Gratuite':FCFA(shipCost)}</span></div></div>)}
            <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:20}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:16,marginBottom:14,color:C.gold}}>Méthode de paiement</h3>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:14}}>
                {[{id:'fedapay',l:'Mobile Money',c:'#e8a020',s:'MTN · Moov · Wave · Orange',icon:<Phone size={17}strokeWidth={1.5}/>},{id:'card',l:'Carte Visa/MC',c:'#635BFF',s:'Cartes internationales',icon:<CreditCard size={17}strokeWidth={1.5}/>}].map(m=>(<button key={m.id}type="button"onClick={()=>setPay(m.id)}style={{border:`0.5px solid ${pay===m.id?m.c:C.border}`,borderRadius:4,padding:'12px 8px',background:pay===m.id?`${m.c}12`:'#0f0f0f',cursor:'pointer',textAlign:'center',fontFamily:"'Jost',sans-serif",transition:'all .2s'}}><div style={{marginBottom:4,display:'flex',justifyContent:'center',color:pay===m.id?m.c:C.muted}}>{m.icon}</div><div style={{fontWeight:500,fontSize:11,color:pay===m.id?m.c:C.white,letterSpacing:1}}>{m.l}</div><div style={{fontSize:10,color:C.muted,marginTop:2,fontWeight:300}}>{m.s}</div></button>))}
              </div>
              <div style={{display:'flex',alignItems:'center',gap:8,padding:'9px 12px',background:'#081008',borderRadius:4,border:`0.5px solid ${C.green}33`}}><Lock size={11}color={C.green}/><span style={{fontSize:11,color:C.green,fontWeight:400}}>Paiement sécurisé via FedaPay — SSL 256-bit</span></div>
            </div>
          </div>
          <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:18,position:'sticky',top:76}}>
            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:18,marginBottom:13}}>Votre commande</h3>
            <div style={{display:'flex',flexDirection:'column',gap:7,marginBottom:13,maxHeight:160,overflowY:'auto'}}>
              {cart.map((item,idx)=>(<div key={item._key||idx}style={{display:'flex',gap:8,alignItems:'flex-start'}}><div style={{width:28,height:28,borderRadius:4,background:C.card2,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,flexShrink:0}}>{item.emoji}</div><div style={{flex:1}}><p style={{fontWeight:500,fontSize:12,color:C.white}}>{item.name}</p>{item.variantLabel&&<p style={{fontSize:10,color:C.gold,fontWeight:300}}>{item.variantLabel}</p>}<p style={{color:C.muted,fontSize:10,fontWeight:300}}>×{item.qty}</p></div><span style={{fontWeight:700,fontSize:11,color:C.gold,flexShrink:0}}>{FCFA(item.price*item.qty)}</span></div>))}
            </div>
            <div style={{height:0.5,background:C.border,marginBottom:10}}/>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:6}}><span style={{color:C.muted,fontWeight:300}}>Sous-total</span><span style={{fontWeight:600}}>{FCFA(subtotal)}</span></div>
            <div style={{display:'flex',justifyContent:'space-between',fontSize:12,marginBottom:10}}><span style={{color:C.muted,fontWeight:300}}>Livraison</span><span style={{fontWeight:600,color:shipCost===0?C.green:C.white}}>{shipCost===0?'Gratuite':FCFA(shipCost)}</span></div>
            <div style={{display:'flex',justifyContent:'space-between',fontFamily:"'Cormorant Garamond',serif",fontWeight:700,fontSize:18,marginBottom:14}}><span>Total</span><span style={{color:C.gold}}>{FCFA(total)}</span></div>
            <button type="button"className="btn"onClick={doPay}disabled={proc}style={{width:'100%',background:proc?'#1a1a1a':`linear-gradient(135deg,${C.goldD},${C.gold})`,color:proc?C.muted:C.bg,border:'none',borderRadius:2,padding:'12px',fontWeight:500,fontSize:11,cursor:proc?'not-allowed':'pointer',fontFamily:"'Jost',sans-serif",display:'flex',alignItems:'center',justifyContent:'center',gap:8,letterSpacing:2,textTransform:'uppercase'}}>{proc?<><Spin s={14}/>Traitement…</>:`Confirmer — ${FCFA(total)}`}</button>
            <button type="button"onClick={()=>setPage('cart')}style={{width:'100%',marginTop:8,background:'none',border:'none',color:C.muted,fontWeight:400,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",padding:'6px',display:'flex',alignItems:'center',justifyContent:'center',gap:4,letterSpacing:1}}><ChevronLeft size={12}/>Retour au panier</button>
          </div>
        </div>
      </div>)}

      {/* SUCCESS */}
      {page==='ok'&&(<div style={{maxWidth:480,margin:'60px auto',padding:'0 22px',textAlign:'center'}}><div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:'44px 36px'}}><div style={{width:72,height:72,borderRadius:'50%',background:`linear-gradient(135deg,${C.goldD},${C.gold})`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px',animation:'glow 2s ease infinite'}}><CheckCircle size={34}color={C.bg}strokeWidth={2}/></div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,fontWeight:600,marginBottom:10}}>Commande confirmée</h2><p style={{color:C.muted,fontSize:13,lineHeight:1.7,marginBottom:20,fontWeight:300}}>Merci ! Un email de confirmation vous a été envoyé.</p><button type="button"className="btn"onClick={()=>{setPage('home');setForm({name:'',email:'',tel:''});setErrs({});}}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'12px 28px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:2,textTransform:'uppercase'}}>Retour à l'accueil</button></div></div>)}

      {/* RÉSERVATION SUCCESS */}
      {page==='resOk'&&(<div style={{maxWidth:480,margin:'60px auto',padding:'0 22px',textAlign:'center'}}><div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:'44px 36px'}}><div style={{width:72,height:72,borderRadius:'50%',background:`linear-gradient(135deg,#1a6b2e,${C.green})`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 20px',animation:'glow 2s ease infinite'}}><Calendar size={34}color="#fff"strokeWidth={2}/></div><h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontWeight:600,marginBottom:10,color:C.green}}>Réservation enregistrée</h2><p style={{color:C.muted,fontSize:13,lineHeight:1.7,marginBottom:8,fontWeight:300}}>Votre acompte de <strong style={{color:C.gold}}>{FCFA(lastRes?.acompte||0)}</strong> a été reçu.</p><p style={{color:C.muted,fontSize:13,lineHeight:1.7,marginBottom:20,fontWeight:300}}>Notre équipe vous contactera pour finaliser votre {lastRes?.product||'réservation'}.</p><a href={`${WA}?text=Bonjour%20S-Mall%2C%20j'ai%20réservé%20${encodeURIComponent(lastRes?.product||'')}%20-%20Réf:%20${lastRes?.resId||''}`}target="_blank"rel="noreferrer"style={{display:'flex',alignItems:'center',justifyContent:'center',gap:10,background:'#25d366',color:'#fff',borderRadius:2,padding:'12px',fontWeight:500,fontSize:11,textDecoration:'none',fontFamily:"'Jost',sans-serif",marginBottom:12,letterSpacing:1.5,textTransform:'uppercase'}}><MessageCircle size={17}/>Contacter via WhatsApp</a><button type="button"onClick={()=>setPage('home')}style={{background:'transparent',color:C.muted,border:`0.5px solid ${C.border}`,borderRadius:2,padding:'10px 22px',fontWeight:400,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:'uppercase'}}>Retour à l'accueil</button></div></div>)}

      {/* CONTACT */}
      {page==='contact'&&(<div style={{maxWidth:760,margin:'0 auto',padding:'36px 28px',animation:'fadeUp .35s ease'}}>
        <p style={{color:C.gold,fontWeight:400,letterSpacing:4,textTransform:'uppercase',fontSize:9,marginBottom:5}}>✦ Nous contacter</p>
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,fontWeight:600,marginBottom:8}}>Parlons-nous</h2>
        <GL/>
        <div className="col1"style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
          <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:24}}>
            <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:19,marginBottom:5}}>Message</h3>
            <p style={{fontSize:12,color:C.muted,marginBottom:18,fontWeight:300}}>Réponse sous 24h</p>
            {ctSent?<div style={{textAlign:'center',padding:'28px 0'}}><CheckCircle size={40}color={C.green}strokeWidth={1.5}style={{margin:'0 auto 10px',display:'block'}}/><p style={{fontWeight:500,fontSize:13,color:C.green,marginBottom:6}}>Message envoyé !</p><button type="button"onClick={()=>setCtSent(false)}style={{background:'none',border:`0.5px solid ${C.border}`,color:C.muted,borderRadius:2,padding:'7px 14px',cursor:'pointer',fontFamily:"'Jost',sans-serif",fontSize:11,letterSpacing:1.5,textTransform:'uppercase'}}>Envoyer un autre</button></div>
            :<div style={{display:'flex',flexDirection:'column',gap:11}}>
              {[['name','Votre nom *','text'],['email','Email','email'],['tel','Téléphone','tel']].map(([f,l,t])=>(<div key={f}><label style={{fontSize:10,fontWeight:500,color:C.muted,display:'block',marginBottom:4,letterSpacing:1.5,textTransform:'uppercase'}}>{l}</label><input type={t}value={ctForm[f]}onChange={e=>setCtForm(cf=>({...cf,[f]:e.target.value}))}placeholder={l.replace(' *','')}style={{width:'100%',background:C.card2,border:`0.5px solid ${C.border}`,borderRadius:4,padding:'10px 13px',color:C.white,fontSize:13,fontFamily:"'Jost',sans-serif",outline:'none',boxSizing:'border-box',fontWeight:300}}/></div>))}
              <div><label style={{fontSize:10,fontWeight:500,color:C.muted,display:'block',marginBottom:4,letterSpacing:1.5,textTransform:'uppercase'}}>Message *</label><textarea value={ctForm.message}onChange={e=>setCtForm(cf=>({...cf,message:e.target.value}))}placeholder="Votre message…"rows={4}style={{width:'100%',background:C.card2,border:`0.5px solid ${C.border}`,borderRadius:4,padding:'10px 13px',color:C.white,fontSize:13,fontFamily:"'Jost',sans-serif",outline:'none',resize:'none',boxSizing:'border-box',fontWeight:300}}/></div>
              <button type="button"className="btn"onClick={async()=>{if(!ctForm.name.trim()||!ctForm.message.trim()){notify('Remplissez nom et message',C.red);return;}try{await sb.from('messages').insert({from_name:ctForm.name,from_email:ctForm.email||'N/A',subject:`Contact S-Mall — ${ctForm.name}`,message:ctForm.tel?`Tél: ${ctForm.tel}\n\n${ctForm.message}`:ctForm.message});setCtSent(true);setCtForm({name:'',email:'',tel:'',message:''});notify('✦ Message envoyé !');}catch(e){notify("Erreur lors de l'envoi.",C.red);}}}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'11px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",display:'flex',alignItems:'center',justifyContent:'center',gap:7,letterSpacing:2,textTransform:'uppercase'}}><Send size={13}/>Envoyer</button>
            </div>}
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:16}}>
            <div style={{background:'linear-gradient(135deg,#080f08,#0d180d)',border:'0.5px solid #25d36633',borderRadius:2,padding:22}}><h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:19,marginBottom:7,color:'#25d366'}}>WhatsApp</h3><p style={{fontSize:12,color:C.muted,marginBottom:16,lineHeight:1.6,fontWeight:300}}>Réponse instantanée tous les jours !</p><a href={`${WA}?text=Bonjour%20S-Mall`}target="_blank"rel="noreferrer"style={{display:'flex',alignItems:'center',justifyContent:'center',gap:9,background:'#25d366',color:'#fff',borderRadius:2,padding:'12px',fontWeight:500,fontSize:11,textDecoration:'none',fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:'uppercase'}}><MessageCircle size={17}/>Ouvrir WhatsApp</a></div>
            <div style={{background:C.card,border:`0.5px solid ${C.borderOr}`,borderRadius:2,padding:22,flex:1}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:600,fontSize:19,marginBottom:5}}>Laisser un avis</h3>
              {rvSent?<div style={{textAlign:'center',padding:'16px 0'}}><p style={{fontWeight:500,color:C.green,fontSize:13}}>Merci !</p><p style={{color:C.muted,fontSize:11,marginTop:5,fontWeight:300}}>Publié après validation.</p></div>
              :<div style={{display:'flex',flexDirection:'column',gap:10,marginTop:14}}>
                <input value={rvForm.name}onChange={e=>setRvForm(r=>({...r,name:e.target.value}))}placeholder="Votre nom *"style={{width:'100%',background:C.card2,border:`0.5px solid ${C.border}`,borderRadius:4,padding:'9px 13px',color:C.white,fontSize:13,fontFamily:"'Jost',sans-serif",outline:'none',boxSizing:'border-box',fontWeight:300}}/>
                <div><p style={{fontSize:10,fontWeight:500,color:C.muted,marginBottom:5,letterSpacing:1.5,textTransform:'uppercase'}}>Note *</p><div style={{display:'flex',gap:4}}>{[1,2,3,4,5].map(s=>(<button key={s}type="button"onClick={()=>setRvForm(r=>({...r,rating:s}))}style={{fontSize:22,background:'none',border:'none',cursor:'pointer',color:s<=rvForm.rating?C.gold:'#2a2a2a',transition:'transform .1s'}}onMouseEnter={e=>e.target.style.transform='scale(1.2)'}onMouseLeave={e=>e.target.style.transform='scale(1)'}>★</button>))}</div></div>
                <textarea value={rvForm.comment}onChange={e=>setRvForm(r=>({...r,comment:e.target.value}))}placeholder="Commentaire *"rows={3}style={{width:'100%',background:C.card2,border:`0.5px solid ${C.border}`,borderRadius:4,padding:'9px 13px',color:C.white,fontSize:13,fontFamily:"'Jost',sans-serif",outline:'none',resize:'none',boxSizing:'border-box',fontWeight:300}}/>
                <button type="button"className="btn"onClick={async()=>{if(!rvForm.name.trim()||!rvForm.comment.trim()){notify('Remplissez tous les champs',C.red);return;}try{await sb.from('reviews').insert({client_name:rvForm.name,rating:rvForm.rating,comment:rvForm.comment,approved:false});setRvSent(true);notify('✦ Avis envoyé !');}catch(e){notify("Erreur lors de l'envoi",C.red);}}}style={{background:`linear-gradient(135deg,${C.goldD},${C.gold})`,color:C.bg,border:'none',borderRadius:2,padding:'10px',fontWeight:500,fontSize:11,cursor:'pointer',fontFamily:"'Jost',sans-serif",display:'flex',alignItems:'center',justifyContent:'center',gap:6,letterSpacing:2,textTransform:'uppercase'}}><Star size={12}/>Publier</button>
              </div>}
            </div>
          </div>
        </div>
      ...
      {/* CIRCUITS */}
      {page==='circuits'&&(<CircuitsPage data={circuitData}circuits={products.filter(p=>p.cat==='circuit'&&p.active!==false)}allImages={allImages}onBook={(pack)=>setBookCircuit(pack)}/>)}
            ...
            window.FedaPay.init({public_key,transaction:{token},onComplete:async r=>{
              ...
              try{
                await fetch(EDGE,{method:'POST',headers:{'Content-Type':'application/json'},
                  body:JSON.stringify({
                    to:'agencesgroup23@gmail.com',
                    subject:'Circuit paye — '+info.packName,
                    html:'<div style="font-family:sans-serif;background:#080808;color:#f4ede0;padding:20px;"><h2 style="color:#c8a84b;">Circuit S-Mall</h2><p>'+info.packName+' — '+info.name+' / '+info.tel+' — '+info.date+' — '+FCFA(amt)+'</p></div>',
                  })
                });
              }catch(e){console.warn('Email admin failed');}
              if(info.email&&info.email!=='N/A'){
                try{
                  await fetch(EDGE,{method:'POST',headers:{'Content-Type':'application/json'},
                    body:JSON.stringify({
                      to:info.email,
                      subject:'Circuit confirme — '+info.packName,
                      html:'<div style="font-family:sans-serif;background:#080808;color:#f4ede0;padding:20px;border-radius:12px;"><h1 style="color:#c8a84b;">S-MALL</h1><p>Bonjour '+info.name+', acompte de '+FCFA(amt)+' pour circuit '+info.packName+' (depart: '+info.date+') recu. Ref : '+resId+'</p></div>',
                    })
                  });
                }catch(e){console.warn('Email client failed');}
              }
      ...
      {/* FOOTER */}
      <footer style={{borderTop:`0.5px solid ${C.borderOr}`,padding:'24px 36px',display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:12,marginTop:24,background:C.card}}>
        <div style={{display:'flex',alignItems:'center',gap:10}}>
        ...
        <div style={{display:'flex',gap:20,flexWrap:'wrap',alignItems:'center'}}>
          <span className="lnk"onClick={()=>go('privacy')}style={{fontSize:9,color:C.muted,letterSpacing:1.5,textTransform:'uppercase',fontWeight:400}}>Confidentialité</span>
